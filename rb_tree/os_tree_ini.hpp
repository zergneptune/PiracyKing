...
{
    auto p_new_node = std::make_shared<KeyNode>(data, Color::RED, 1);
    ...
    else if (p_new_node->data < temp->data)
    {
    ...
    //维护size
    auto p_temp = PA(p_new_node);
    while (p_temp != NIL_NODE)
    {
        ++p_temp->size;
        p_temp = PA(p_temp);
    }
...
void RBTree<T>::remove_node(std::shared_ptr<KeyNode> del_node)
{
    ...
    if (del_node->left == NIL_NODE)//1. 待删除节点没有左孩子
    {
        ...
        transplant(del_node, fixup_node);
    }
    ...
    //维护size
    auto modify_size_node = PA(fixup_node);//fixup_node可能为NIL_NODE
    while (modify_size_node != NIL_NODE)
    {
        --modify_size_node->size;
        modify_size_node = PA(modify_size_node);
    }
    ...
    while(!temp_st.empty())
    {
        ...
        else
        {
            while (!temp_st.empty())
            {
                ...
                temp_st.pop();
                std::cout << temp_node->data << "(" << get_color(temp_node->color) << ", size:" << temp_node->size << "), ";
...
template<typename T>
std::shared_ptr<typename RBTree<T>::KeyNode> RBTree<T>::search_k(int k)
{
    auto p_temp = m_root;
    while (p_temp != NIL_NODE) {
        if (k == p_temp->left->size + 1)
        {
            return p_temp;
        }
        if (k < p_temp->left->size + 1)
